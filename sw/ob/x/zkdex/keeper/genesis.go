...
func (k Keeper) InitGenesis(ctx context.Context, genState types.GenesisState) error {
	...
	// 5. Set order nullifiers used
	for _, nullifier := range genState.OrderNullifierUsed {
		if err := k.SetOrderNullifierUsed(ctx, nullifier); err != nil {
			return err
		}
	}

	// 6. Set deposits processed
...
func (k Keeper) ExportGenesis(ctx context.Context) (*types.GenesisState, error) {
	...
	orderNullifiers, err := k.GetAllOrderNullifiersUsed(ctx)
	if err != nil {
		return nil, err
	}
